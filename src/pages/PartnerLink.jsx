...
import { Send, Link as LinkIcon, Heart, Copy, Check } from 'lucide-react';
...
const PartnerLink = ({ setUser }) => {
  ...
  const handleInvite = async (e) => {
    ...
    try {
      const res = await client.post('/couple/invite', { email });
      ...
      <div className="bg-white w-full max-w-md rounded-3xl shadow-xl overflow-hidden relative">
        ...
        <div className="flex border-b border-gray-100">
          <button 
            ...
            className={`flex-1 p-4 font-medium text-sm transition-colors ${mode === 'invite' ? 'text-love-500 bg-love-50' : 'text-gray-400 hover:text-gray-600'}`}
          >
            Invite Partner
          </button>
          <button 
            onClick={() => setMode('join')}
            className={`flex-1 p-4 font-medium text-sm transition-colors ${mode === 'join' ? 'text-love-500 bg-love-50' : 'text-gray-400 hover:text-gray-600'}`}
          >
            I Have a Code
          </button>
        </div>
                        ...
                        {!generatedCode ? (
                            <form onSubmit={handleInvite} className="space-y-4">
                                <input 
                                    ...
                                    className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-love-500 focus:ring-2 focus:ring-love-200 outline-none transition-all"
                                    required
                                />
                                <button 
                                    disabled={loading}
                                    className="w-full bg-love-500 hover:bg-love-600 text-white font-medium py-3 rounded-xl transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                                >
                            ...
                            <div className="bg-love-50 border border-love-100 rounded-xl p-4 text-center">
                                ...
                                <div 
                                    ...
                                    className="flex items-center justify-between bg-white border border-dashed border-love-300 rounded-lg px-4 py-3 cursor-pointer hover:border-love-500 transition-colors group"
                                >
                        ...
                        <form onSubmit={handleJoin} className="space-y-4">
                            <div className="relative">
                                <input 
                                    ...
                                    className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-purple-500 focus:ring-2 focus:ring-purple-200 outline-none transition-all font-mono tracking-widest text-center uppercase"
                                    required
                                    maxLength={8}
                                />
                            </div>
